...
if (isset($_GET['delmedreviewnote'])){
    $del_r=$_GET['delmedreviewnote'];
    $del_quer=mysqli_query($conn,"DELETE FROM medreviewnote WHERE id='$del_r' ");  
    if ($del_quer) {
        
        echo "<script type='text/javascript'>alert('medreviewnote Deleted! Successfully!')</script>";   
        header("location:medreviewnote.php");
    }
    else {
        
        ?>
        <script type="text/javascript">alert('Delete medreviewnote failed!')</script>
        <?php
        header("location:medreviewnote.php");
    }
